...
const answer_question = (req, res) => {
    ...
    // 1. Fetch the question to find out which item it belongs to
    itemModel.getQuestionById(question_id, (err, question) => {
        if (err) return res.sendStatus(500);
        if (!question) return res.sendStatus(404);

        // 2. Fetch the item to see who the creator is
        itemModel.getItemById(question.item_id, (err, item) => {
            if (err) return res.sendStatus(500);
            
            // 3. Authorization: Only the creator of the item can answer
            if (item.creator_id !== user_id) {
                return res.status(403).json({ error: "Only the item creator can answer questions" });
            }
